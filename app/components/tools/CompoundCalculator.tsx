...
export function CompoundCalculator() {
  ...
  const t = useTranslations('tools.calc.ci');
      ...
      {result && (
        <div className="space-y-3">
          <div className="rounded-2xl border border-white/8 bg-white/3 p-5">
            ...
            <div className="text-3xl font-bold text-emerald-400">{formatKrw(result.finalTotal)}{tc('won')}</div>
          </div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <div className="rounded-2xl border border-white/8 bg-white/3 p-5">
              ...
              <div className="text-xl font-bold">{formatKrw(result.totalPrincipal)}{tc('won')}</div>
            </div>
            <div className="rounded-2xl border border-white/8 bg-white/3 p-5">
              <div className="mb-2 flex items-center gap-2 text-xs font-medium text-white/50">
              ...
              <div className="text-xl font-bold text-emerald-400">
                +{formatKrw(result.totalReturn)}{tc('won')}
          ...
          <div className="rounded-2xl border border-white/8 bg-white/3 p-5">
            <div className="mb-4 text-xs font-medium text-white/50">{t('assetGrowth')}</div>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={result.chartData} margin={{ top: 5, right: 5, left: 5, bottom: 5 }}>
                  <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)" />
                  <XAxis dataKey="year" tick={{ fill: 'rgba(255,255,255,0.3)', fontSize: 11 }} tickLine={false} axisLine={false} unit={t('yearUnit')} />
                  ...
                  <Tooltip
                    ...
                    formatter={(value: number | undefined, name: string | undefined) => [
                      `${formatKrw(value ?? 0)}${tc('won')}`,
                      name === 'total' ? t('totalAssets') : t('principal'),
                    ]}
