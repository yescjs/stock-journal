...
export function CelebrationEffects({ events, darkMode, onComplete }: CelebrationEffectsProps) {
  ...
  const queueCount = Math.max(pendingEvents.length - 1, 0);

  const handleDismiss = () => {
    if (!currentEvent) return;
    setDismissedIds((prev) => (prev.includes(currentEvent.id) ? prev : [...prev, currentEvent.id]));
      ...
      >
        ...
        >
          ...
          <motion.p
            ...
            className={`text-center mb-6 ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}
          >
          ...
          >
            ...
            >
              {queueCount > 0 ? `다음 (${queueCount}개 남음)` : '완료!'}
            </Button>
          </motion.div>
